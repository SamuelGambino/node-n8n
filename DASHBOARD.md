...
## Если ИИ возвращает `parts` вместо текста

Некоторые ИИ-узлы n8n возвращают объект сообщения, а не строку, например:

```json
{
  "parts": [
    { "text": "client_id;…\n301;…", "thoughtSignature": "служебные данные" }
  ]
}
```

В финальный ответ webhook нельзя передавать весь такой объект как строковое значение `final_report_csv`. Нужно извлечь только `parts[0].text` (или объединить все `parts[].text`) и передать получившийся **полный** CSV. Аналогично для `audit_md` передавайте чистый текст Markdown, а не сериализованный объект ответа ИИ.

Backend дополнительно умеет распаковать эту распространённую обёртку сам, но не может восстановить CSV, если ИИ остановился до завершения последней строки. Для CSV увеличьте лимит выходных токенов ИИ и в prompt явно потребуйте вернуть весь документ без сокращения, вводного текста и Markdown-обрамления. При неполном результате страница теперь покажет диагностическое сообщение вместо неясной ошибки CSV-парсера.
